...
def buck(str):
      ...
      for i in range(20):
          ...
          scale = round(float(item['sell_min_price']) / (float(item['goods_info']['steam_price_cny']) * 0.87), 4)
          c.execute('select * from scale01')
          ...
          try:
            sql="insert into scale01 values({id},'{name}',{buffMinPrice},{steamMinPrice},{scale})".format(id=id,name=name,buffMinPrice=buffMinPrice,steamMinPrice=steamMinPrice,scale=scale)
            print(sql)
            c.execute(sql)  # 执行sql语句
            conn.commit()  # 提交数据库操作
          except sqlite3.IntegrityError:
            sql="update scale01 set name='{name}',buff_min_price={buffMinPrice},steam_min_Price={steamMinPrice},scale={scale} where id={id}".format(id=id,name=name,buffMinPrice=buffMinPrice,steamMinPrice=steamMinPrice,scale=scale)
            print(sql)
            c.execute(sql)  # 执行sql语句
            conn.commit()  # 提交数据库操作
